...
def setup_and_submit_displacements(staticpoint_path, submit=True, dry_run=False):
    ...
    staticpoint_path : str or Path
        Path to the staticpoint directory (e.g., "phonopy_bilayer_examples/MoS2_TaTe2_3R_staticpoint")
    submit : bool
    ...
    if staticpoint_path.is_absolute() and staticpoint_path.exists():
        ...
        # Assume it's a bilayer name in phonopy_bilayer_examples
...
Examples:
  ...
  python3 setup_displacements.py --all
    ...
    parser.add_argument(
        "staticpoint",
        nargs="?",
        help="Staticpoint directory name or path (e.g., 'MoS2_TaTe2_3R_staticpoint' or 'phonopy_bilayer_examples/MoS2_TaTe2_3R_staticpoint')"
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Process all staticpoint directories in phonopy_bilayer_examples"
    )
    parser.add_argument(
        "--no-submit",
        action="store_true",
